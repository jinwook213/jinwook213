...
class Clark: # 일단 CN을 아는 상황에 대한 코드!
    ...
    def __init__(self, Tc, K, area):
        
        self.Tc = Tc
        self.K = K
        self.area = area
        self.time_step = 100 # 100개로 쪼개는 게 디폴트
        self.length = int(Tc*self.time_step*100)
    ...
    def make_Clark(self):
        ...
        Clark['UH_0.01hr'] = (Clark['IUH'] + Clark['IUH_shift'])/2

        # 1 h 단위도 그림으로 확인
        # import matplotlib.pyplot as plt
        # print('UH_0.01hr')
        # plt.plot(Clark['time'][:count_IUH], Clark['UH_0.01hr'][:count_IUH], label='UH_0.01hr')
        # plt.legend()
        # plt.xlabel('Time (h)')
        ...
        Clark['UH_0.01hr_mm'] = Clark['UH_0.01hr']/10       
       
        # 유량 volume 확인(m3 단위)
        vol_dis = sum(Clark['UH_0.01hr_mm'])/time_step*3600 # m3 단위
    ...
    def Routing(self, rain_array, CN_value):
        ...
        '''rain array를 0.01 h 단위로 강우 단위는 mm/0.01 h'''
        import pandas as pd
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        ## convolution! 
        discharge_table = pd.DataFrame( np.arange( (len(rain_array) + count_IUH) ) /time_step, columns=['time']) #*time_step
        discharge_table['Q'] = 0
        discharge = np.zeros( len(discharge_table) ) #len(discharge_table) )
        for i in range(len(rain_table)):
            for j in range(count_IUH):
                discharge[i+j] += rain_table['eff'][i] * Clark['UH_0.01hr_mm'][j] #*time_step
