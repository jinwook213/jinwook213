...
class Clark: # 일단 CN을 아는 상황에 대한 코드!
    ...
    def make_Clark(self):
        ...
        # 개수만큼 나누어 주기! 
        Clark['UH_1hr'] = sum_UH/time_step    

        # 1 h 단위도 그림으로 확인
        import matplotlib.pyplot as plt
        print('UH_1hr')
        plt.plot(Clark['time'][:count_IUH], Clark['UH_1hr'][:count_IUH], label='UH_1hr')
        plt.legend()
        plt.xlabel('Time (h)')
        ...
        Clark['UH_1hr_mm'] = Clark['UH_1hr']/10       
       
        # 유량 volume 확인(m3 단위)
        vol_dis = sum(Clark['UH_1hr_mm'])/time_step*3600 # m3 단위
    ...
    def Routing(self, rain_array, CN_value):
        ...
        for i in range(len(rain_table)):
            for j in range(count_IUH):
                discharge[(i)*time_step+j] += rain_table['eff'][i] * Clark['UH_1hr_mm'][j]
